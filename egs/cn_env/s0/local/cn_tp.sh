...
# CN Text Processing
if [ $# -ne 3 ]; then
    echo "cn_tp.sh [--nj <nj>] [--has-key true/false]  <vocab.txt> <text.txt> <working_dir>"
    exit 1;
fi
...
if [ $nj -eq 1 ]; then
    echo "TN..."
    local/cn_tn.py $opts --to_upper $text $dir/${name}_tn
    echo "TN done."

    echo "WS..."
    local/cn_ws.py $opts $dir/jieba.vocab $dir/${name}_tn $dir/${new_text}
    echo "WS done."
else
    # parallel tn & ws
    echo "TN..."
    ...
    for f in `ls $dir/${name}_*`; do
        local/cn_tn.py $opts --to_upper $f ${f}_tn >& ${f}_tn.log &
    done
    wait
    echo "TN done."
    ...
    for f in `ls $dir/${name}_*_tn`; do
        local/cn_ws.py $opts $dir/jieba.vocab $f ${f}_ws >& ${f}_ws.log &
    done
    wait
    echo "WS done."
